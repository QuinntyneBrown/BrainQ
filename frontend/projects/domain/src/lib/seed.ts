import { BqAgenda, BqEntity, BqHeatLevel } from './models';

export const SEED_ENTITIES: readonly BqEntity[] = [
  // ── People ──────────────────────────────────────────────────
  {
    id: 'p_amelia',
    type: 'Person',
...
export const SEED_AGENDA: BqAgenda = {
  date: 'Friday, May 1',
...
export const SEED_HEATMAP: BqHeatLevel[][] = (() => {
  const weeks = 18;
  const out: BqHeatLevel[][] = [];
  for (let w = 0; w < weeks; w++) {
    const col: BqHeatLevel[] = [];
    for (let d = 0; d < 7; d++) {
